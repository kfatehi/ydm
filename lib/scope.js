...
function Scope(options) {
  ...
  console.log("Initializing "+this.home);
  ...
  if ( ! config) {
    ...
    this.localStorage.setItem('_numLinks', 0);
  }
...
Scope.prototype = {
  ...
  // Same as applyConfig except it will always destroy a container if it exists
  reapplyConfig: function (config, cb) {
    config.removeContainerIfExists = true
    this.applyConfig(config, cb)
  },
  applyConfig: function (config, cb) {
    ...
    if (_.isArray(config.env)) {
      config.createOpts.Env = config.env
    } else if (_.isObject(config.env)) {
      config.createOpts.Env = _.map(config.env, function (value, key) {
        return key+"="+value
      })
    }
    if (config.volumes) {
    ...
    if (config.start && config.start.Links) {
      console.log(config.start.Links)
      throw ''
    }
