...
function Scope(home, name) {
  ...
  if (dockerHost) {
    var uri = URI.parse(dockerHost)
    // this is not bulletproof -- you might be using dns, etc
    local = _.contains(getMyIPAddresses(), uri.host)
    docker = new Docker(uri);
...
Scope.prototype = {
  applyConfig: function (config, cb) {
    ...
    console.log(container.defaultOptions.start);
...
function volumePath(name) {
  ...
  if (local) {
    ...
    console.warn("Ensure remote docker host writable dir "+volPath+" !");
  }
};
...
function ensure(scope, config, cb) {
  ...
  container.inspect(function (err, res) {
    if (err) {
      ...
      var hostConfig = res["HostConfig"];
      console.log(hostConfig);
      if (config.volumes) {
        console.log("Checking volume configuration");
        if (hostConfig === null) {
          cb(new Error("Volumes did not mount!"))
        } else {
          _.each(config.volumes, function (containerPath, volName) {
            if (hostConfig[containerPath] !== volumePath(volName)) {
