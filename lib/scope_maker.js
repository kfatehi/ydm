...
module.exports = {
  ...
  makeScope: function(scopeName, argv, cb) {
    var appHome = process.env.YDM_HOME || argv.ydmhome || this.mkdir(path.join(home, '.ydm'))
      , scopesDir = this.mkdir(path.join(appHome, 'scopes'))
      , scopeHome = null
      , namespace = null
    if (argv.namespace) {
      namespace = new Scope({
